...
The project **authenticationRestAPI** is a Bonita REST API extension for **Bonita {short-bonita-version}** version  **community** written in `groovy` language.
...
[source,bash]
----
git clone https://your.github.com/authenticationRestAPI.git
cd authenticationRestAPI/
mvn package
----

The build should produce a zip archive under the `target/` folder named `authenticationRestAPI-1.0.0-SNAPSHOT.zip`
...
Take the built artifact `authenticationRestAPI-1.0.0-SNAPSHOT.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])
