...
class ReportesDAO {
	...
	public Result generarReporteAdmitidosPropedeutico(String jsonData) {
		...
		try {
			...
			def titulos = ["Nombre","No. Matrícula","Email","Teléfonos","Celular","Ciudad","Preparatoria","Promedio","Clave carrera","Nombre de propedeutico","Tipo Admisión","Solicitud Admisión","Pago de Admisión","Examen Admisión","Resultado de admisión","Rechazado","Fecha admisión","Pago curso","Tipo de Estudiante"]
			if(object.encabezado) {
				fw.write((titulos.join(",")+"\r\n").getBytes());
				Row headersRow = sheet.createRow(rowCount);
				++rowCount;
