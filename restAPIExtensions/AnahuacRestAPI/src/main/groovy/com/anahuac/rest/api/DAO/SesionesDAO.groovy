...
class SesionesDAO {
	...
	public Result getLstAspirantes(Long campus_pid) {
		...
		try {
			List < Map < String, Object >> rows = new ArrayList < Map < String, Object >> ();
			closeCon = validarConexion()
			pstm = con.prepareStatement("SELECT correoelectronico,dt.idbanner from solicituddeadmision sda inner join detallesolicitud dt on dt.caseid::bigint=sda.caseid where sda.catcampus_pid=? order by sda.persistenceid desc limit 100 ")
