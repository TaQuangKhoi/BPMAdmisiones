...
= AnahuacBecasRestAPI
API REST para administrar Becas

The project **AnahuacBecasRestAPI** is a Bonita REST API extension for **Bonita {short-bonita-version}** version  **community** written in `groovy` language.
...
[source,bash]
----
git clone https://your.github.com/AnahuacBecasRestAPI.git
cd AnahuacBecasRestAPI/
mvn package
----

The build should produce a zip archive under the `target/` folder named `AnahuacBecasRestAPI-1.0.0-SNAPSHOT.zip`
...
Take the built artifact `AnahuacBecasRestAPI-1.0.0-SNAPSHOT.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])
