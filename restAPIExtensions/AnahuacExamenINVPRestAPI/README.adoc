...
= AnahuacExamenINVP REST API
API REST para administrar resourceName

The project **AnahuacExamenINVPRestAPI** is a Bonita REST API extension for **Bonita {short-bonita-version}** version  **community** written in `groovy` language.
...
[source,bash]
----
git clone https://your.github.com/AnahuacExamenINVPRestAPI.git
cd AnahuacExamenINVPRestAPI/
mvn package
----

The build should produce a zip archive under the `target/` folder named `AnahuacExamenINVPRestAPI-1.0.0-SNAPSHOT.zip`
...
Take the built artifact `AnahuacExamenINVPRestAPI-1.0.0-SNAPSHOT.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])
