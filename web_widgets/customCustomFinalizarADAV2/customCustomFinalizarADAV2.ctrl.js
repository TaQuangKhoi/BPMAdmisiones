function PbButtonCtrl($scope, $http, $location, $log, $window, localStorageService, modalService) {
    ...
    function validateTask() {
        let isValid = false;
        if ($scope.properties.validate === true) {
            isValid = true;
        } else {
            isValid = false;
        }
        ...
        if (!isValid) {
            swal($scope.properties.titleMessage, $scope.properties.errorMessage, "warning");
        }
            ...
            .success(function(data, status) {
                ...
                setTimeout(() => {
                    ...
                    if ($scope.properties.targetUrlOnSuccess && method !== 'GET') {
                        swal("Autodescripción completada.", "Redireccionando...", "success");
                        redirectIfNeeded();
                    }
                    closeModal($scope.properties.closeOnSuccess);
                }, 2000);
                ...
                // notifyParentFrame({ message: 'success', status: status, dataFromSuccess: data, dataFromError: undefined, responseStatusCode: status});
                // if ($scope.properties.targetUrlOnSuccess && method !== 'GET') {
                //   swal("Autorescripción completada.", "Redireccionando...", "success");
                //   setTimeout(() => {
                //     redirectIfNeeded();
                //   }, 5000);
                // }
