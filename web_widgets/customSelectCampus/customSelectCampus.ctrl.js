function PbSelectCtrl($scope, $parse, $log, widgetNameFactory, $timeout, $window, $element) {
  ...
  function comparatorDescripcion(initialValue, item) {
    return angular.equals(initialValue.descripcion, ctrl.getValue(item).descripcion);
  }
  ...
  this.findSelectedItem = function (items) {
    return items.filter(comparatorDescripcion.bind(null, $scope.properties.value))
      .map(function (item) {
