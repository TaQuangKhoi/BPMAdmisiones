        ...
        debugger
			...
			if ($scope.properties.dataToChange2.clave && $scope.properties.dataToChange2.descripcion) {
				if ($scope.properties.processId) {
                    ...
                    var req2 = {
                        method: 'GET',
                        url: "/API/extension/AnahuacRestGet?url=getValidarClave&p=0&c=10&tabla=CatTeGustaLeer&clave=" + $scope.properties.dataToChange2.clave + "&id=" + $scope.properties.dataToChange2.persistenceId
                        };
			...
			if ($scope.properties.dataToChange2.lstCatTeGustaLeerInput[0].clave && $scope.properties.dataToChange2.lstCatTeGustaLeerInput[0].descripcion) {
				if ($scope.properties.processId) {


                    var req2 = {
                        method: 'GET',
                        url: "/API/extension/AnahuacRestGet?url=getValidarClave&p=0&c=10&tabla=CatTeGustaLeer&clave=" + $scope.properties.dataToChange2.lstCatTeGustaLeerInput[0].clave + "&id="
                    };
                return $http(req2).success(function(datos, status) {
                    if(datos.data[0] === true){
                         var prom = doRequest('POST', '../API/bpm/process/' + $scope.properties.processId + '/instantiation', $scope.properties.userId).then(function () {
				...
				if (!$scope.properties.dataToChange2.lstCatTeGustaLeerInput[0].descripcion) {
					swal("¡Aviso!","Falto capurar informacion en: Descripción", "warning");
				}
				if (!$scope.properties.dataToChange2.lstCatTeGustaLeerInput[0].clave) {
