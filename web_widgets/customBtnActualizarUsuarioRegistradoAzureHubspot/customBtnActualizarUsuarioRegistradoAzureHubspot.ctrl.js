function PbButtonCtrl($scope, $http, $location, $log, $window, localStorageService, modalService,blockUI) {
     ...
     function doRequest(method, url, params) {
        ...
        return $http(req).success(function(data, status) {
            ...
            /*if(
                ...
                || $scope.properties.dataToSend.estatussolicitud === "Transferido"
    ...
    function doRequestGuardar(method, url, params) {
        ...
        return $http(req).success(function(data, status) {
            ...
            $scope.properties.dataFromError = undefined;
            notifyParentFrame({ message: 'success', status: status, dataFromSuccess: data, dataFromError: undefined, responseStatusCode: status});
            $scope.createOrUpdateUsuariosRegistrados($scope.properties.JSONUsuarioRegistrado.correoelectronico, method);
        }).error(function(data, status) {
            $scope.properties.dataFromError = data;
            $scope.properties.responseStatusCode = status;
            $scope.properties.dataFromSuccess = undefined;
     ...
     * @return {void}
     */
     function obtenerDocumentos(caseId) {
        vm.busy = true;
        let url = "../API/extension/AnahuacAzureRest?url=obtenerDocumentos&p=0&c=10&caseId=" + caseId;
        var req = {
            method: "POST",
            url: url,
            data: {}
