function PbButtonCtrl($scope, $http, $location, $log, $window, localStorageService, modalService) {
                ...
                doRequestCallBack("GET", "/bonita/API/extension/AnahuacRestGet?url=getIdbanner&idbanner=" + $scope.properties.dataToSend.detalleSolicitudInput.idBanner, {}, {}, function(datos) {
                    if ($scope.properties.dataToSend.detalleSolicitudInput.isCurpValidado === false && $scope.properties.isMexicano) {
                        ...
                        if ($scope.properties.objDetalleSolicitud.idBanner !== "") {
                            ...
                            if ($scope.properties.dataToSend.detalleSolicitudInput.idBanner === null || $scope.properties.dataToSend.detalleSolicitudInput.idBanner.length < 8 || $scope.properties.dataToSend.detalleSolicitudInput.idBanner.length > 8) {
    ...
    function submitTask() {
        ...
        if (id) {
            ...
            doRequest2('POST', $scope.properties.urlComentario, params)
    ...
    function doRequest2(method, url, params) {
        vm.busy = true;
        var comentarios = angular.copy($scope.properties.comentarioInfo);
        comentarios.comentario = $scope.properties.observacion
        var req = {
            method: method,
            url: url,
            data: angular.copy(comentarios),
            params: params
        };
