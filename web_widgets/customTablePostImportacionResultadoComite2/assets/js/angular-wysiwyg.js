    ...
    return {
      	template: '<div>' +				
    			...
    			'<div class="btn-group btn-group-sm wysiwyg-btn-group-margin">' +
    				...
    				'<button title="Justify" tabindex="-1" type="button" unselectable="on" class="btn btn-default" ng-click="format(\'justifyFull\')" ng-class="{ active: isJustified}"><i class="	glyphicon glyphicon-align-justify"></i></button>' +
    			'</div>' +
    			'<div class="btn-group btn-group-sm wysiwyg-btn-group-margin">' +
     ...
     if (attrs.enableBootstrapTitle === "true" && attrs.enableBootstrapTitle !== undefined)
    	...
    	function getHiliteColor(){
        	...
        	textarea.on('click keyup focus mouseup', function(){
        		$timeout(function(){
        			...
        			scope.isJustified = scope.cmdState('justifyFull'); 
    	...
    	scope.$on('$viewContentLoaded', function(){
         ...
         scope.$watch(function () {
            return scope.element.innerHTML;
          }, function (newVal) {
            if (angular.isDefined(newVal) && newVal!=null){
                ngModelController.$viewValue=newVal;
                scope.value=newVal;
            }else{
                scope.element = document.getElementById(scope.textareaId);
            $("#"+scope.textareaId).keyup(function(event) {
                      	scope.$apply(function readViewText() {
    			var html = textarea.html();
                ...
                scope.value=scope.element.innerHTML;
    			ngModelController.$setViewValue(scope.element.innerHTML);
    		});
                    });
    	...
    	scope.keyPress = function(e){
    		console.error("verga que chingon")
    	}
