function PbButtonCtrl($scope, $http, $location, $log, $window, localStorageService, modalService) {
    ...
    function startProcess() {
		...
		console.log("test");
		if(isId && isDescripcion && isCatRelacionCampusTipo && isClave){
			...
			if(isValidList){
				...
				if ($scope.properties.processId) {
					...
					var req = {
					    method: "GET",
					    url: "../API/extension/AnahuacRestGet?url=getValidarIdBanner&p=0&c=10&tabla=CatPropedeuticoGral&idBanner=" + idBanner + "&id="
					};
					return $http(req)
			...
			else if(!isCatRelacionCampusTipo){
