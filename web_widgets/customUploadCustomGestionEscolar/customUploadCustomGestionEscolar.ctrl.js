function PbUploadCtrl($scope, $sce, $element,$http, widgetNameFactory, $timeout, $log, gettextCatalog) {
    ...
    this.forceSubmit = function(event) {
        ...
        if (event.target.files[0].type.includes("image")) {
            ...
            swal("¡Formato no valido!", "Solo puede agregar imagenes.", "warning");
 ...
 $scope.upload= function(){
    ...
    $scope.previewPDF=function(){
        ...
        Swal.fire({
      title: '<strong>Previsualizar imagen</strong>',
      width: 600,
      height:800,
      confirmButtonColor: '#231F20',
      html:
        '<object style="height: 80vh; width:100%" data="'+$scope.properties.urlretorno+'" type="image"> <embed src="'+$scope.properties.urlretorno+'" type="image" /> </object>',
    ...
    async function Main() {
       const file = $('input[name="imagen"]').get(0).files[0];
