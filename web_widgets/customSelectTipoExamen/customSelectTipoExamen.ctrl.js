function PbSelectCtrl($scope, $parse, $log, widgetNameFactory, $timeout, $window, $element) {
  ...
  this.findSelectedItem = function (items) {
    return items.filter(comparator.bind(null, $scope.properties.value))
      .map(function (item) {
          if($scope.properties.update!=="" && $scope.properties.update !== ctrl.getValue(item)){
            swal("¡Aviso!",`seguro desea cambiar el tipo de examen, esto borrara los resultados capturados.`,"warning")
          }
