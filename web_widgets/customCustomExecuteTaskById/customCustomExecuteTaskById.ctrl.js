function PbButtonCtrl($scope, $http, $location, $log, $window, localStorageService, modalService) {
    ...
    function getCurrentTaskId(){
        ...
        var req = {
            method: "GET",
            url: "/API/bpm/humanTask?c=50&f=state%3Dready&o=displayName+ASC&p=0&f=caseId=" +  $scope.properties.caseId
        };
  
        return $http(req).success(function(data, status) {
            let psicometricoEncontrado = false;
            if(data.length > 0){
                for(let i = 0; i < data.length; i++){
                    if(data.length > 0){
                        if(data[i].name === "Reporte psicométrico"){
                            if($scope.properties.taskId !== data[i].id){
                                psicometricoEncontrado = true;
                                $scope.properties.taskId = data[i].id;
                            } else {
                                // getCurrentTaskId();
                                if($scope.count < 100){
                                    $scope.count++;
                ...
                if(psicometricoEncontrado === false){
                    // getCurrentTaskId();
                    if($scope.count < 100){
                        $scope.count++;
                        getCurrentTaskId();
