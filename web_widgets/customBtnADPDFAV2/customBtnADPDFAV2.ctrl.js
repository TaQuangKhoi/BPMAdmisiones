function PbButtonCtrl($scope, modalService, $http, blockUI, $q, $filter) {
  ...
  'use strict';
    ...
    $scope.generatePDF = function() {
        ...
        //  ----------------------------------------- TERCERA SECCIÓN  ----------------------------------------- 
        doc.setFillColor(228, 212, 200);
        doc.rect(10, (height / 2) + 14, 190, 75, 'F');

        doc.setFontSize(fontTitle);
        doc.setFont(undefined, 'bold');
        doc.text(margenPrimeraFila, (height / 2) + 20, 'Información escolar');

        //Encabezados
        doc.setFontSize(fontText);
        doc.setFont(undefined, 'bold');
        doc.text(margenPrimeraFila, (height / 2) + 30, 'Nombre:');
        doc.text(margenFilaIntermedia, (height / 2) + 30, 'Año inicio:');
        doc.text(margenSegundaFila, (height / 2) + 30, 'Año fin:');

        doc.text(margenPrimeraFila, (height / 2) + 60, 'Promedio:');
        doc.text(margenFilaIntermedia, (height / 2) + 60,  '¿Has estudiado en el extranjero?:');

        //Respuestas
        doc.setFont(undefined, 'normal');
        doc.text($scope.properties.autodescripcionV2.catEstudiadoExtranjero.descripcion, margenFilaIntermedia, (height / 2) + 65);        
        ...
        doc.text($scope.properties.autodescripcionV2.materiasCalifAltas, respuestasPrimeraFila, (height / 2) - 79);
        doc.text($scope.properties.autodescripcionV2.materiasTeGustan, respuestasPrimeraFila, (height / 2) - 56);

        doc.text($scope.properties.autodescripcionV2.materiasNoTeGustan, respuestasPrimeraFila, (height / 2) - 5);
        doc.text($scope.properties.autodescripcionV2.materiasCalifBajas, respuestasPrimeraFila, (height / 2) - 29);
        ...
        //  ----------------------------------------- QUINTA SECCIÓN  ----------------------------------------- 
        doc.setFillColor(228, 212, 200);
        doc.rect(10, (height / 2) + 36, 194, 90, 'F');
        ...
        doc.text(margenPrimeraFila, (height / 2) + 70, 'Nombre de la organización o empresa donde trabajaste:');
        doc.text(margenPrimeraFila, (height / 2) + 85, 'Nombre de la organización o empresa donde trabajas actualmente:');
        doc.text(margenPrimeraFila, (height / 2) +98 , '¿Sientes que tu experiencia de trabajo te ayudó a elegir tu carrera?:');
        ...
        doc.text("N/A", margenPrimeraFila, (height / 2) + 77);            
        }else{
        doc.text(($scope.properties.autodescripcionV2.catExperienciaAyudaCarrera.descripcion == null ? "N/A" : $scope.properties.autodescripcionV2.catExperienciaAyudaCarrera.descripcion), margenPrimeraFila, (height / 2) + 77);            
        }
        doc.text(($scope.properties.autodescripcionV2.empresaTrabajas == null || $scope.properties.autodescripcionV2.empresaTrabajas == "" ? "N/A" :$scope.properties.autodescripcionV2.empresaTrabajas), margenPrimeraFila, (height / 2) + 89);
        doc.text(($scope.properties.autodescripcionV2.empresaTrabajaste == null || $scope.properties.autodescripcionV2.empresaTrabajaste == "" ? "N/A" :$scope.properties.autodescripcionV2.empresaTrabajaste), margenPrimeraFila, (height / 2) + 110);
        ...
        doc.setFontSize(fontText);
        doc.setFont(undefined, 'bold');
        doc.text(margenPrimeraFila, (height / 2) - 120, 'Adicional a tu lengua materna, ¿hablas o estudias algún otro idioma?');
        doc.text(margenPrimeraFila, (height / 2) - 100, 'Idiomas');
        doc.text(margenFilaIntermedia, (height / 2) - 100, 'Nivel');
        
      
        //  ----------------------------------------- OCTAVA SECCIÓN  ----------------------------------------- 
        doc.setFillColor(228, 212, 200);
        doc.rect(10, (height / 2) - 50, 194, 180, 'F');

        doc.setFontSize(fontSubTitle);
        doc.setFont(undefined, 'bold');
        doc.text(margenPrimeraFila, (height / 2) - 40, 'Salud');

       //Encabezados
        doc.setFontSize(fontText);
        doc.setFont(undefined, 'bold');
        doc.text(margenPrimeraFila, (height / 2) - 20, '¿Te consideras una persona saludable?:');
        doc.text(margenPrimeraFila, (height / 2) - 5, '¿Vives en situación de discapacidad?:');
        doc.text(margenPrimeraFila, (height / 2) + 10, '¿Qué tipo de discapacidad?:');
        doc.text(margenPrimeraFila, (height / 2) + 20, '¿Tienes algún problema de salud que necesite atención médica continua?:');
        doc.text(margenPrimeraFila, (height / 2) + 30, 'Describe brevemente:');
        doc.text(margenPrimeraFila, (height / 2) + 50, '¿Has recibido alguna terapia?:');
        
        //Respuestas
        doc.setFont(undefined, 'normal');
        doc.text($scope.properties.autodescripcionV2.catPersonaSaludable.descripcion, respuestasPrimeraFila, (height / 2) - 15);
        doc.text($scope.properties.autodescripcionV2.catVivesEstadoDiscapacidad.descripcion, margenSegundaFila, (height / 2) - 2);

        doc.text($scope.properties.autodescripcionV2.tipoDiscapacidad, respuestasPrimeraFila, (height / 2) + 14);
        doc.text($scope.properties.autodescripcionV2.catProblemasSaludAtencion.descripcion, respuestasPrimeraFila, (height / 2) + 24);
        doc.text($scope.properties.autodescripcionV2.problemasSaludAtencionContinua, margenSegundaFila, (height / 2) - 35);
        ...
        doc.setFontSize(fontSubTitle);
        doc.setFont(undefined, 'bold');
        doc.text(margenPrimeraFila, (height / 2) + 70, 'Entorno familiar');
        
        //Encabezados
        doc.setFontSize(fontText);
        doc.setFont(undefined, 'bold');
        doc.text(margenPrimeraFila, (height / 2) + 80, '¿Cómo describirías a tu familia?');
        doc.text(margenPrimeraFila, (height / 2) + 95, '¿Si pudieras cambiar algo de tu familia, qué sería?');
        doc.text(margenPrimeraFila, (height / 2) + 110, '¿Con quién de tu familia tienes una mejor relación?');
        
    
        doc.addPage();
        //  ----------------------------------------- DECÍMA SECCIÓN  ----------------------------------------- 
        doc.text(margenPrimeraFila, (height / 2) - 135, 'Cuándo tienes un problema, ¿con quién lo platicas?');
        ...
        //  ----------------------------------------- ONCEAVA SECCIÓN  ----------------------------------------- 
        doc.setFillColor(228, 212, 200);
        doc.rect(10, (height / 2) - 60, 190, 200, 'F');

        doc.setFontSize(fontSubTitle);
        doc.setFont(undefined, 'bold');
        doc.text(margenPrimeraFila, (height / 2) - 50, 'Entorno social');
